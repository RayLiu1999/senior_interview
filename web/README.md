...
- W5 提供匿名同步碼：`POST /api/progress/sync` 會以 attempt ID 去重並合併離線資料；讀寫既可使用 `Authorization: Bearer <syncToken>`，GET 也暫時相容 `?syncToken=...`。

## 同步部署注意事項

同步 API 的預設儲存位置是 `web/.data/progress.json`，也可以用 `INTERVIEW_PROGRESS_STORE` 指定持久化路徑。API 預設限制每個 client 每分鐘 30 次請求、同步 state 2 MiB，並回傳 `Cache-Control: no-store`；可用 `INTERVIEW_PROGRESS_RATE_LIMIT`、`INTERVIEW_PROGRESS_RATE_WINDOW_SECONDS`、`INTERVIEW_PROGRESS_MAX_BYTES` 與 `INTERVIEW_PROGRESS_MAX_REQUEST_BYTES` 調整。這是單機、可替換的 file-backed adapter；若部署到多個 instance，仍應替換成共享資料庫或持久化 KV，並加入 token 撤銷、加密與正式帳號身份。
