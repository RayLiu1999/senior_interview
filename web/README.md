...
- W5 提供匿名同步碼：`POST /api/progress/sync` 會以 attempt ID 去重並合併離線資料，`GET /api/progress/sync?syncToken=...` 可讀回紀錄。

## 同步部署注意事項

同步 API 的預設儲存位置是 `web/.data/progress.json`，也可以用 `INTERVIEW_PROGRESS_STORE` 指定持久化路徑。這是單機、可替換的 file-backed adapter；若部署到多個 instance，應替換成共享資料庫或持久化 KV，並為同步碼加入速率限制、撤銷、加密與正式帳號身份。
