...
- **Concept ID**: `concept.nodejs.tooling.module-resolution`
- **Learning Objectives**:
  - `LO-1`: 能追蹤 Node.js package resolution、exports、imports、symlink 與 hoisting。
  - `LO-2`: 能診斷 duplicate dependency、peer dependency、workspace、lock drift 與 runtime mismatch。
  - `LO-3`: 能設計 deterministic install、artifact provenance、security scan 與 rollback。
- **Prerequisites**: npm／package.json、CommonJS／ESM 與 dependency graph
- **Quick Quiz**: [Node.js Quick Quiz：node_modules 與 resolution](../../../../QUIZ/07_Node.js.md)
